...
class PanierController extends AbstractController
{
    ...
    #[Route('/Panier/show', name: 'Panier_show', methods: ['GET', 'POST'])]
    public function show(PanierRepository $rep): Response
    {
        ...
        $paniers = $rep->findByUser($user); // Pass the user object
    
   
    
        // Check if paniers is empty
        if (empty($paniers)) {
            throw new \Exception('Paniers not found for user: ' . $user->getUsername());
        }
...
public function sendSms(Request $request): Response
{
    ...
    $paniers = $this->getDoctrine()->getRepository(Panier::class)->findByUser($user);
...
{
    $user = $this->getUser(); // Retrieve the user from the session
    $panier = $this->getDoctrine()->getRepository(Panier::class)->findByUser($user);
